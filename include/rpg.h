...
int Attack(personagem *p)
{
	int damage;
    	...
    	// Assumindo que 0 = fac„o, 1 = escopeta, 3 = revolver, default = inaptid√£o, tal que:
...
void History(personagem *p){
	...
	while(1){
		// o level determina a parte da histÛria que ser· contada
		// 0 - inicio do jogo
		...
		case 0:
			...
			Dialogue("\t\t\t VocÍ È um cara que sua filha fica Doente ", "green", 0);
			Dialogue("\t\t\tVocÍ decide ent„o partir em uma jornada em busca de uma cura para ela", "green", 0);
			Dialogue("\t\t\tAssim, armado com um velho fac„o", "green", 0);
